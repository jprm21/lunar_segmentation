...
def parse_args():
    parser = argparse.ArgumentParser(description="Train cross-scene lunar segmentation model")
    parser.add_argument(
        "--augmentation-profile",
        default="default",
        choices=sorted(AUGMENTATION_PROFILES.keys()),
        help="Augmentation profile to use for training dataset.",
    )
...
def main():
    args = parse_args()
    profile_cfg = AUGMENTATION_PROFILES[args.augmentation_profile]
    print(f"[INFO] Augmentation profile activo: {args.augmentation_profile}")
    ...
    for epoch in range(EPOCHS):
        ...
        model.train()
        train_loss = 0.0
        train_dataset.reset_epoch_stats()
        ...
        if args.augmentation_profile == "rock_boost":
            print(
                "Copy-paste de rocas aplicado "
                f"{train_dataset.copy_paste_applied_count} veces en la época"
            )
