"""
conversion.py
Pipeline: PyTorch -> ONNX -> SavedModel (onnx-tf) -> TFLite INT8

IMPORTANTE:
Este modelo fue entrenado SIN normalización ImageNet.
El dataset usa TF.to_tensor(), por lo que el input esperado es float32 en [0,1].
NO se aplica (x - mean) / std.

INT8:
- calibration usando representative dataset
- input/output int8
"""
...
def convert_to_tflite_int8(dataset: LuSNARDataset) -> None:
    ...
    # Representative dataset: USAR input real del modelo (float32 en [0,1])
    def representative_dataset_gen():
        limit = min(CALIBRATION_SAMPLES, len(dataset))
        for idx in range(limit):
            image, _ = dataset[idx]

            # Dataset ya entrega float32 [0,1] en CHW
...
def run_previews(dataset: LuSNARDataset) -> None:
    ...
    print(f"  Input quant:  scale={in_scale}, zero={in_zero}")
    print(f"  Output quant: scale={out_scale}, zero={out_zero}")

    for idx in range(min(PREVIEW_SAMPLES, len(dataset))):
        image, mask_gt = dataset[idx]

        # Dataset entrega float32 [0,1] en CHW
        image_chw = image.numpy().astype(np.float32)

        if is_nhwc_in:
            model_input = np.transpose(image_chw, (1, 2, 0))
        else:
            model_input = image_chw

        model_input = np.expand_dims(model_input, axis=0).astype(np.float32)

        # Cuantizar input float32 -> int8
        ...
        # Dequantizar output int8 -> float32
